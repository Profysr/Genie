...
function computeRange(sprints) {
  const dates = sprints
    .flatMap((s) => [parseDate(s.start_date), parseDate(s.end_date)])
    .filter(Boolean);
...
export default function RoadmapPage() {
  ...
  const sprintsByProject = useMemo(
    () =>
      projects.reduce((acc, p, i) => {
        acc[p.id] = (sprintResults[i]?.data || []).filter(
  ...
  const allSprints = useMemo(
    () => Object.values(sprintsByProject).flat(),
    [sprintsByProject],
  );
  ...
  const { start: baseStart, end: baseEnd } = useMemo(
    () => computeRange(allSprints),
    [allSprints],
  );
  ...
  const header = useMemo(
    () => buildHeader(rangeStart, rangeEnd, pxPerDay, zoom),
    ...
    () =>
      projects.map((p) => {
        ...
        return {
          project: p,
          sprints,
          totalH: PRJ_HDR_H + sprints.length * SPRINT_H,
        };
      }),
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-3 border-b flex-shrink-0 bg-card/60 flex items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Sprint timeline across all projects · drag to move · drag right edge
            to extend
          </p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        {/* Left panel */}
        <div
          className="flex-shrink-0 flex flex-col border-r border-border bg-card z-10 shadow-sm"
          style={{ width: LEFT_W }}
        >
          <div
                    ...
                    <div key={project.id}>
                      ...
                      {sprints.map((s, si) => (
                        <div
                          ...
                          style={{
                            top: yStart + PRJ_HDR_H + si * SPRINT_H,
                            height: SPRINT_H,
                            width: LEFT_W - 1,
                          }}
                        >
                          <span
                            ...
                            style={{
                              backgroundColor:
                                COLORS[sprints.indexOf(s) % COLORS.length],
                            }}
                          />
        ...
        >
          ...
          <div
            className="relative"
            style={{ width: totalWidth, height: totalBodyH, minWidth: "100%" }}
          >
                  ...
                  <div key={project.id}>
                    ...
                    {sprints.map((sprint, si) => (
                      <SprintBar
                        ...
                        dragPreview={
                          dragPreview?.sprintId === sprint.id
                            ? dragPreview
                            : null
                        }
...
function SprintBar({
  ...
  const w =
    dType === "resize" ? Math.max(pxPerDay, baseW + dd * pxPerDay) : baseW;

  const isDragging = !!dragPreview;

  // Live dates for tooltip
  const previewStart =
    dType === "move" ? dateKey(addDays(startDate, dd)) : sprint.start_date;
  const previewEnd =
    dType === "resize"
      ? dateKey(addDays(endDate, dd))
      : dType === "move"
        ? dateKey(addDays(endDate, dd))
        : sprint.end_date;
      ...
      const loop = () => {
        ...
        if (Math.abs(dx) > DRAG_THRESHOLD) {
          ...
          if (rc && rightRef.current) {
            let vel = 0;
            if (rc.right - mouseX < EDGE_ZONE)
              vel = Math.round(
                (1 - (rc.right - mouseX) / EDGE_ZONE) * MAX_SPEED,
              );
            if (mouseX - rc.left < EDGE_ZONE)
              vel = -Math.round(
                (1 - (mouseX - rc.left) / EDGE_ZONE) * MAX_SPEED,
              );
            if (vel !== 0) {
          ...
          if (deltaDays !== 0) {
            if (type === "move") {
              updateSprint.mutate({
                ...
                end_date: dateKey(
                  addDays(parseDate(dragRef.current.origEnd), deltaDays),
                ),
              });
            } else {
              const newEnd = addDays(
                parseDate(dragRef.current.origEnd),
                deltaDays,
              );
              if (newEnd > parseDate(dragRef.current.origStart)) {
                updateSprint.mutate({
                  sprintId: sprint.id,
                  end_date: dateKey(newEnd),
                });
              }
            }
          }
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            hovered && !isDragging ? `0 2px 8px ${color}66` : undefined,
        }}
      ...
      {hovered && !isDragging && (
        <div className="absolute left-0 bottom-full mb-2 z-30 bg-popover border border-border rounded-md shadow-xl px-3 py-2.5 min-w-[200px] pointer-events-none">
