...
import {
  ...
  Globe, Lock, History, Edit3, Check, X, ArrowLeft,
} from "lucide-react";
      ...
      <div className="w-56 flex-shrink-0 border-r flex flex-col bg-sidebar-bg/50">
        <div className="flex items-center justify-between px-3 py-3 border-b">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Wiki</span>
          </div>
          <button
        ...
        {creatingPage && (
          <form onSubmit={handleCreatePage} className="px-3 py-2 border-b">
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Escape") setCreatingPage(false); }}
            />
            <div className="flex gap-1 mt-1.5">
              <button type="submit" className="flex-1 text-xs bg-primary text-primary-foreground rounded py-1 font-medium">Create</button>
              <button type="button" onClick={() => setCreatingPage(false)} className="text-xs text-muted-foreground px-2">Cancel</button>
            </div>
          </form>
        )}
        ...
        {!pageId ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground text-sm">Select a page from the sidebar or create a new one.</p>
              <Button size="sm" className="mt-4" onClick={() => setCreatingPage(true)}>
            ...
            <div className="flex items-center justify-between px-8 py-4 border-b flex-shrink-0">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  ...
                  {page.created_by && ` · by ${page.created_by.full_name || page.created_by.email}`}
                </p>
              </div>
              <div className="flex items-center gap-1.5 ml-4">
                <button
                  onClick={() => updatePage.mutate({ is_public: !page.is_public })}
                  className={cn(
                  ...
                  title={page.is_public ? "Public — click to make private" : "Private — click to make public"}
                >
                  {page.is_public ? <Globe className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
...
function WikiTreeNode({ page, projectId, workspaceSlug, activePageId, depth }) {
  ...
  return (
    <div>
      ...
      >
        {hasChildren ? (
          <span onClick={(e) => { e.stopPropagation(); setOpen(o => !o); }} className="flex-shrink-0">
  ...
  return (
    <div className="w-72 border-l flex flex-col flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto">
        {revisions.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-8">No revisions yet.</p>
        ) : revisions.map(rev => (
          <div
            key={rev.id}
            className={cn("px-4 py-3 border-b cursor-pointer hover:bg-accent transition-colors", preview?.id === rev.id && "bg-primary/5")}
            onClick={() => setPreview(preview?.id === rev.id ? null : rev)}
          >
            <p className="text-xs font-medium">{rev.author?.full_name || rev.author?.email}</p>
            <p className="text-xs text-muted-foreground">{format(new Date(rev.created_at), "MMM d, yyyy · h:mm a")}</p>
            {preview?.id === rev.id && (
              <Button size="sm" variant="outline" className="mt-2 w-full text-xs" onClick={() => onRestore(rev.content)}>
