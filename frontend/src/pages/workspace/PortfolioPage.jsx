          ...
          {[
            {
              label: "On Track",
              val: onTrack,
              cls: "text-emerald-600 bg-emerald-50 dark:bg-emerald-950 dark:text-emerald-300",
            },
            {
              label: "At Risk",
              val: atRisk,
              cls: "text-yellow-600  bg-yellow-50  dark:bg-yellow-950  dark:text-yellow-300",
            },
            {
              label: "Off Track",
              val: offTrack,
              cls: "text-red-600     bg-red-50     dark:bg-red-950     dark:text-red-300",
            },
          ].map((s) => (
            <div
              key={s.label}
        ...
        {projects.length === 0 ? (
          ...
          projects.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="bg-card border border-border rounded-md p-5 shadow-card hover:shadow-card-hover transition-shadow cursor-pointer"
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden mb-1.5">
                  <div
                    className={cn(
                      ...
                      cfg.dot.replace("bg-", "bg-").replace("-400", "-400"),
                    )}
                    style={{ width: `${pct}%` }}
                  />
                </div>
