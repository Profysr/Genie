      ...
      <div className="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium min-w-[180px] text-center">
            {format(weekDate, "MMM d")} – {format(addWeeks(weekDate, 1), "MMM d, yyyy")}
          </span>
          <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50">
                  ...
                  {days.map(day => (
                    <th key={day} className="text-center px-3 py-3 font-medium text-muted-foreground">
                      <div>{format(new Date(day), "EEE")}</div>
                      <div className="text-xs font-normal">{format(new Date(day), "MMM d")}</div>
                    </th>
                  ))}
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  rows.map((row, i) => (
                    <tr key={row.user.id} className={cn("border-t", i % 2 === 0 ? "bg-card" : "bg-muted/20")}>
                      ...
                      {days.map(day => {
                        ...
                        return (
                          <td key={day} className="text-center px-3 py-3">
                            {secs > 0 ? (
                              <span className="text-foreground font-medium">{formatDuration(secs)}</span>
                            ) : (
                  ...
                  <tr className="border-t bg-muted/50 font-semibold">
                    ...
                    {totalPerDay.map((secs, i) => (
                      <td key={i} className="text-center px-3 py-3">
                        {secs > 0 ? formatDuration(secs) : <span className="text-muted-foreground/40">—</span>}
                      </td>
                    ))}
                    <td className="text-center px-4 py-3 text-primary">{formatDuration(grandTotal)}</td>
                  </tr>
                </tfoot>
              )}
            </table>
