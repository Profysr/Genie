...
const DATA_SOURCES = [
  {
    ...
    label: "Tasks",
    icon: CheckSquare,
    desc: "Task counts by status, priority, assignee, or type",
  },
  {
    id: "time_entries",
    label: "Time Entries",
    icon: Clock,
    desc: "Hours logged per member or project",
  },
  {
    id: "velocity",
    label: "Sprint Velocity",
    icon: TrendingUp,
    desc: "Completed tasks & story points per sprint",
  },
  {
    id: "throughput",
    label: "Throughput",
    icon: Activity,
    desc: "Tasks completed over time",
  },
];
...
function ChartPreview({ chartType, rawData }) {
  ...
  if (!data.length) {
    return (
      <div className="h-56 flex flex-col items-center justify-center text-center gap-2 bg-muted/30 rounded-md">
        <BarChart2 className="w-8 h-8 text-muted-foreground/40" />
        <p className="text-sm text-muted-foreground">
          No data — configure source and filters, then save
        </p>
      </div>
    );
  }
  ...
  if (chartType === "kpi") {
    ...
    return (
      <div className="h-56 flex flex-col items-center justify-center bg-muted/20 rounded-md">
  ...
  if (chartType === "table") {
    return (
      <div className="h-56 overflow-auto rounded-md border border-border">
  ...
  if (chartType === "horizontal_bar") {
    ...
    return (
      <ResponsiveContainer
        width="100%"
        height={Math.max(220, sorted.length * 36 + 20)}
      >
        <RBarChart
          data={sorted}
          layout="vertical"
...
function ShareModal({ report, workspaceSlug, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-card border border-border rounded-md shadow-2xl w-full max-w-sm mx-4 p-6 z-10">
...
function ReportEditor({ report, workspaceSlug, onBack, onDelete }) {
  ...
  const [config, setConfig] = useState(
    report.config || { chart_type: "bar", data_source: "tasks" },
  );
  ...
  const {
    ...
    refetch,
  } = useReportData(workspaceSlug, report.id);
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="bg-card border border-border rounded-md p-5 shadow-card min-h-[320px]">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {DATA_SOURCES.find((d) => d.id === config.data_source)
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Build, save, and share custom charts from your project data.
            </p>
          </div>
          <button
            onClick={onCreate}
              ...
              {reports.map((r) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    {DATA_SOURCES.find((d) => d.id === r.config?.data_source)
                      ?.label || "Custom"}{" "}
                    ·{" "}
                    {CHART_TYPES.find((t) => t.id === r.config?.chart_type)
                      ?.label || "Chart"}
                  </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {TEMPLATES.map((tpl) => (
              <button
                ...
                className="bg-card border border-dashed border-border hover:border-primary/40 rounded-md p-4 text-left hover:bg-accent/30 transition-all disabled:opacity-50"
              >
                <p className="text-sm font-medium mb-1">{tpl.name}</p>
                <p className="text-[11px] text-muted-foreground">
                  {
                    DATA_SOURCES.find((d) => d.id === tpl.config.data_source)
                      ?.label
                  }{" "}
                  ·{" "}
                  {
                    CHART_TYPES.find((t) => t.id === tpl.config.chart_type)
                      ?.label
                  }
                </p>
              </button>
