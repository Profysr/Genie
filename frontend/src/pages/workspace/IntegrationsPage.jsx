      ...
      {open && (
        <div className="border-t border-border px-4 py-4 space-y-4 bg-muted/20">
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">
                ...
                <span className="opacity-60">
                  (leave blank to use workspace default)
                </span>
              </label>
              <input
            ...
            <div className="flex gap-2">
              {["compact", "detailed"].map((f) => (
                <button
                  key={f}
                  onClick={() =>
                    setForm((s) => ({ ...s, notification_format: f }))
                  }
                  className={cn(
          ...
          <EventsPicker
            ...
            onChange={(v) => setForm((f) => ({ ...f, enabled_events: v }))}
          />
    ...
    <div className="bg-card border border-border rounded-md shadow-card overflow-hidden">
      <div className="flex items-center justify-between px-6 py-5 border-b border-border">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-md bg-muted flex items-center justify-center text-2xl flex-shrink-0">
            {logo}
          </div>
          <div>
...
function SlackCard({ workspaceSlug, slack, oauthConfigured }) {
  ...
  const { data: channels = [], refetch: loadChannels } = useSlackChannels(
    workspaceSlug,
    { enabled: !!slack },
  );
    ...
    >
      {justConnected && !slack && (
        <div className="text-xs text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 px-3 py-2 rounded-lg">
          ✅ Slack connected successfully! Configure your channel mappings
          below.
        </div>
      )}
        ...
        <div className="space-y-3">
          {!oauthConfigured && (
            <div className="text-xs text-amber-600 bg-amber-50 dark:bg-amber-950/40 px-3 py-2 rounded-lg">
              ⚠️ Slack OAuth is not configured on this server. Ask your admin to
              add <code className="font-mono">SLACK_CLIENT_ID</code> and{" "}
              <code className="font-mono">SLACK_CLIENT_SECRET</code> to the
              backend environment.
            </div>
          )}
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#4A154B] text-white rounded-md text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-40"
          >
            ...
            <div>
              ...
              {slack.incoming_webhook_channel && (
                <p className="text-xs text-muted-foreground">
                  Default channel: #{slack.incoming_webhook_channel}
                </p>
              )}
            </div>
            <button
    ...
    <div className="text-xs text-muted-foreground">
      ...
      {open && (
        <ol className="mt-2 space-y-1 list-decimal list-inside pl-1 leading-relaxed">
          ...
          <li>
            Under <b>OAuth & Permissions</b>, add the redirect URL:{" "}
            <code className="bg-muted px-1 rounded">
              YOUR_BACKEND_URL/api/integrations/slack/oauth/callback/
            </code>
          </li>
          <li>
            Add bot token scopes:{" "}
            <code className="bg-muted px-1 rounded">
              chat:write, channels:read, commands, users:read.email,
              incoming-webhook
            </code>
          </li>
          <li>
            Under <b>Slash Commands</b>, create{" "}
            <code className="bg-muted px-1 rounded">/jcn</code> pointing to{" "}
            <code className="bg-muted px-1 rounded">
              YOUR_BACKEND_URL/api/integrations/slack/events/
            </code>
          </li>
          <li>
            Under <b>Interactivity</b>, set the request URL to{" "}
            <code className="bg-muted px-1 rounded">
              YOUR_BACKEND_URL/api/integrations/slack/interactive/
            </code>
          </li>
          <li>
            Copy the Client ID, Client Secret, and Signing Secret into your
            backend <code className="bg-muted px-1 rounded">.env</code> file.
          </li>
          <li>
            Restart the backend and click <b>Add to Slack</b> above.
          </li>
        </ol>
      )}
    </div>
