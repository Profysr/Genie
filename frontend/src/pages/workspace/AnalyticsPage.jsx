...
function OverviewTab({ workspaceSlug, projectId }) {
  ...
  const maxS = Math.max(
    1,
    ...(data?.tasks_by_status || []).map((s) => s.count),
  );
  const maxP = Math.max(
    1,
    ...(data?.tasks_by_priority || []).map((p) => p.count),
  );
  const maxW = Math.max(
    1,
    ...(data?.workload || [])
...
function VelocityTab({ workspaceSlug, projectId }) {
  ...
  const { data: vData, isLoading: vLoad } = useVelocity(workspaceSlug, {
    projectId,
  });
  ...
  const { data: bData, isLoading: bLoad } = useBurnup(workspaceSlug, {
    projectId,
    days: 30,
  });
...
function FlowTab({ workspaceSlug, projectId }) {
  ...
  const { data: cfdData, isLoading: cfdLoad } = useCFD(workspaceSlug, {
    projectId,
    days: cfdDays,
  });
  const { data: ltData, isLoading: ltLoad } = useLeadTime(workspaceSlug, {
    projectId,
    days: ltDays,
  });
  const { data: ctData, isLoading: ctLoad } = useCycleTime(workspaceSlug, {
    projectId,
    days: ctDays,
  });
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Select
          ...
          onChange={(v) => setCfdDays(Number(v))}
          options={dayOptions}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <Select
              value={String(ltDays)}
              onChange={(v) => setLtDays(Number(v))}
              options={dayOptions.slice(1)}
            />
          </div>
...
function TeamTab({ workspaceSlug, projectId }) {
  ...
  const { data: hmData, isLoading: hmLoad } = useWorkloadHeatmap(
    workspaceSlug,
    { projectId, days: hmDays },
  );
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-6">
        {activeTab === "overview" && (
          <OverviewTab workspaceSlug={workspaceSlug} projectId={projectId} />
        )}
        {activeTab === "velocity" && (
          <VelocityTab workspaceSlug={workspaceSlug} projectId={projectId} />
        )}
        {activeTab === "flow" && (
          <FlowTab workspaceSlug={workspaceSlug} projectId={projectId} />
        )}
        {activeTab === "team" && (
          <TeamTab workspaceSlug={workspaceSlug} projectId={projectId} />
        )}
      </div>
    </div>
  );
