...
const BAR_FONT  = '500 11px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
const CHIP_FONT = '500 10px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
...
const GanttCanvas = forwardRef(function GanttCanvas(
  ...
  const startAnimLoop = useCallback(() => {
    ...
    const tick = () => {
      ...
      const speed       = wantShow ? 0.20 : 0.15;
  ...
  const draw = useCallback(() => {
    ...
    for (let i = startIdx; i < rows.length; i++) {
      ...
      if (row.type === "sprint") {
        ...
        if (pct < 1) {
          ...
          ctx.fillRect(bxScr + bwAbs * pct, barY, bwAbs * (1 - pct), SPRINT_BAR_H);
        }
      ...
      if (row.type === "task") {
        ...
        // Gradient sheen — top slightly bright, bottom slightly dark
    ...
    // 7.5 ── Hover overlay — drawn after all bars so chips are always on top ────
    ...
    if (hId && hAlpha > 0.01) {
      ...
      if (hRow && hRow.y + hRow.h > sT && hRow.y < sT + H) {
        ...
        if (sd) {
          ...
          // ── Grip handles (2 vertical pill pairs on each edge) ───────────────
          if (bwAbs > 28) {
          ...
          if (!isPrev) {
            ...
            const drawChip = (text, chipX) => {
              ...
              ctx.fillText(text, chipX + chipPad, chipY + chipH / 2 + 4);
              ctx.globalAlpha = 1;
              return chipW;
            };

            // Start chip — right-anchored to bar's left edge
            const swMeasure = ctx.measureText(startStr).width + chipPad * 2;
            const sChipX    = bxScr - swMeasure - 7;
            if (sChipX >= 2) {
              drawChip(startStr, sChipX);
            }

            // End chip — left-anchored to bar's right edge
            const eChipX   = bxScr + bwAbs + 7;
            const ewMeasure = ctx.measureText(endStr).width + chipPad * 2;
            if (eChipX + ewMeasure <= W - 2) {
              drawChip(endStr, eChipX);
            }
