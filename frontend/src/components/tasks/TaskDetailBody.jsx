...
import { X, ChevronDown, ChevronRight, Trash2, Plus, Check, Link2, GitBranch, Search } from "lucide-react";
  ...
  return (
    <div className="relative flex-shrink-0 w-9 h-9">
      <svg viewBox="0 0 24 24" className="w-9 h-9 -rotate-90">
        ...
        <circle
          cx="12" cy="12" r={r} fill="none" strokeWidth="2.5" strokeLinecap="round"
          className={cn("transition-all duration-500", allDone ? "stroke-emerald-500" : "stroke-primary")}
    ...
    <div className="rounded border border-border bg-card overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3.5 py-1.5">
        <div className={cn(
          ...
          <GitBranch className={cn(
            ...
            allDone && total > 0 ? "text-emerald-500" : "text-muted-foreground/40",
          )} />
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          {total > 0 && <CircularProgress done={doneCount} total={total} allDone={allDone} />}
        </div>
      </div>
      ...
      {showPicker && (
        <TaskSearchPicker
          ...
          onSelect={(id) => { attachChild.mutate(id); setShowPicker(false); }}
      ...
      <div className="px-2 pb-2 space-y-0.5">
        {childTasks.map((child) => (
          ...
          >
            ...
            <span className={cn(
              ...
              child.status_detail?.name?.toLowerCase() === "done" && "line-through text-muted-foreground/60",
            )}>
              {child.title}
            </span>
            {child.status_detail && (
              <span
                ...
                style={{ backgroundColor: child.status_detail.color + "20", color: child.status_detail.color }}
              >
        ...
        {canEdit && (
          addingNew ? (
            <form onSubmit={handleAdd} className="flex items-center gap-2 px-2 py-1.5">
        ...
        {total === 0 && !canEdit && (
          <p className="text-xs text-muted-foreground/50 text-center py-3">No child tasks</p>
        )}
      </div>
    </div>
  );
...
export function ChecklistSection({ task, subtasks, canEdit, toggleSubtask, deleteSubtask, createSubtask }) {
  ...
  const handleAdd = (e) => {
    e?.preventDefault();
    if (!newItem.trim()) { setAdding(false); return; }
      ...
      <div className="px-2 pb-2 space-y-0.5">
        ...
        {incomplete.map((sub) => (
          <ChecklistRow
            ...
            sub={sub}
            canEdit={canEdit}
            onToggle={() => toggleSubtask.mutate({ subtaskId: sub.id, is_done: true })}
        ...
        {completed.length > 0 && (
          <>
            ...
            {showCompleted && completed.map((sub) => (
              <ChecklistRow
                key={sub.id}
                sub={sub}
                canEdit={canEdit}
                onToggle={() => toggleSubtask.mutate({ subtaskId: sub.id, is_done: false })}
                onDelete={() => deleteSubtask.mutate(sub.id)}
              />
            ))}
          </>
        ...
        {total === 0 && !canEdit && (
          <p className="text-xs text-muted-foreground/50 text-center py-3">No checklist items</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-2 group px-2 py-1.5 rounded-md hover:bg-accent/40 transition-colors">
      ...
      <span className={cn(
        ...
        sub.is_done ? "line-through text-muted-foreground/50" : "text-foreground",
      )}>
        {sub.title}
      </span>
      {canEdit && (
