...
import { useChildTasks } from "@/hooks/useTaskHierarchy";
...
function getGroupKey(task, groupBy, statuses, sprintsById) {
  switch (groupBy) {
    case "status": {
      ...
      return { id: s?.id || "none", label: s?.name || "No Status", color: s?.color || "#94a3b8" };
    }
    case "assignee":
      return {
        id: task.assignee?.id || "unassigned",
        label: task.assignee ? task.assignee.full_name || task.assignee.email : "Unassigned",
        color: "#6366f1",
      };
    case "priority": {
      const key = task.priority || "no_priority";
      return { id: key, label: key.replace(/_/g, " "), color: PRIORITY_COLORS[key] || "#94a3b8" };
    }
    default: {
      const sprint = sprintsById[task.sprint_id];
      return { id: task.sprint_id || "none", label: sprint?.name || "No Sprint", color: "#8b5cf6" };
    }
  }
}
...
function renderBodyCell(col, task, ctx) {
  const { depth, hasChildren, isExpanded, onToggleExpand, statuses, labelsById, sprintsById, onTaskClick } = ctx;

  if (col === "title") {
    const p = PRI[task.priority] || PRI.no_priority;
    return (
      <td key="title" className="pl-4 pr-3 py-2 w-[40%] cursor-pointer" onClick={() => onTaskClick(task.id)}>
        <div className="flex items-center gap-1.5 group/title">
          {depth > 0 && <span className="flex-shrink-0" style={{ width: depth * 16 }} />}
          {hasChildren ? (
            <button
      ...
      <td key="assignee" className="px-3 py-2">
        {task.assignee ? (
          <div className="flex items-center gap-2">
            <Avatar name={task.assignee.full_name || task.assignee.email} src={task.assignee.avatar} size="xs" />
  ...
  if (col === "sprint") {
    ...
    return (
      <td key="sprint" className="px-3 py-2">
        {sprint ? (
          <span className="text-[11px] text-muted-foreground">{sprint.name}</span>
        ) : (
          <span className="text-muted-foreground/50 text-xs">--</span>
        )}
      </td>
              ...
              <tr className="border-t-2 border-border bg-secondary/50">
                ...
                {visibleColumns.map((c, i) => (
                  <td key={c.id} className={cn("px-3 py-2 text-[11px] text-muted-foreground", i === 0 ? "pl-4" : "")}>
                    {i === 0 ? <span className="font-semibold">{rootTasks.length} tasks</span> : null}
                  </td>
                ))}
              </tr>
            </tfoot>
