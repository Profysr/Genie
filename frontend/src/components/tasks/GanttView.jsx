...
function computeRange(tasks) {
  ...
  if (dates.length === 0)
    return { start: addDays(today, -30), end: addDays(today, 120) };
...
function buildHeader(rangeStart, rangeEnd, zoom, pxPerDay) {
  ...
  if (zoom === "week") {
    ...
    while (ws <= rangeEnd) {
      ...
      const isCur =
        (ref.getMonth() === nowM && ref.getFullYear() === nowY) ||
...
function buildRows(tasks, groupBy, statuses, collapsed) {
  ...
  for (const t of dated) {
    ...
    if (groupBy === "status") {
      ...
      id = t.sprint_detail?.id || "backlog";
      label = t.sprint_detail?.name || "Backlog";
      color = "#8b5cf6";
    }
...
function computeCriticalPath(tasks) {
  ...
  const dp = (id) => {
    ...
    const v =
      (t.blocked_by_ids || []).reduce((acc, bid) => Math.max(acc, dp(bid)), 0) +
      dur(t);
    memo.set(id, v);
    seen.delete(id);
    return v;
...
export default function GanttView({
  ...
  const { start: rangeStart, end: baseRangeEnd } = useMemo(
    () => computeRange(tasks),
    [tasks],
  );
  ...
  const { rows, undated, totalH } = useMemo(
    () => buildRows(tasks, groupBy, statuses, collapsed),
    [tasks, groupBy, statuses, collapsed],
  );
  ...
  const goToday = () => {
    ...
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    rightRef.current.scrollLeft = Math.max(
      0,
      daysBetween(rangeStart, today) * pxPerDay - 200,
    );
  };
  ...
  const barX = (task, deltaDays = 0, type = "move") => {
    ...
    if (!s) return -1;
    return (
      daysBetween(rangeStart, s) * pxPerDay +
      (type === "move" ? deltaDays * pxPerDay : 0)
    );
  };
  const barW = (task, deltaDays = 0, type = "move") => {
    ...
    if (type === "resize")
      return Math.max(pxPerDay, base + deltaDays * pxPerDay);
    return base;
  };
  ...
  const startDrag = useCallback(
    (e, task, type) => {
      ...
      const scrollLoop = () => {
        ...
        const dx =
          drag.current.lastClientX != null
            ? drag.current.lastClientX - drag.current.startX
            : 0;
        ...
        if (moved) {
          ...
          if (fromRight < EDGE_ZONE)
            scrollDelta = Math.round(
              (1 - fromRight / EDGE_ZONE) * MAX_SCROLL_SPEED,
            );
          if (fromLeft < EDGE_ZONE)
            scrollDelta = -Math.round(
              (1 - fromLeft / EDGE_ZONE) * MAX_SCROLL_SPEED,
            );
        ...
        if (drag.current) {
          ...
          if (deltaDays !== 0) {
            ...
            if (drag.current.type === "move") {
              if (drag.current.origStart)
                updates.start_date = dateKey(
                  addDays(parseDate(drag.current.origStart), deltaDays),
                );
              if (drag.current.origDue)
                updates.due_date = dateKey(
                  addDays(parseDate(drag.current.origDue), deltaDays),
                );
            } else {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        {/* ── Left panel ─────────────────────────────────────────────────── */}
        <div
          className="flex-shrink-0 flex flex-col border-r border-border shadow-sm z-10"
          style={{ width: LEFT_W }}
        >
          {/* Aligns with the sticky date header */}
          <div
            className="flex-shrink-0 border-b border-border bg-muted/40 flex items-end px-3 pb-1.5"
            style={{ height: HDR_H }}
          >
          ...
          >
            ...
            {rows
              ...
              .map((r) => (
                <div
                  key={r.id}
                  className="absolute left-0 right-0 bg-muted/20 border-b border-border"
                  style={{ top: r.y, height: GRP_H }}
                />
              ))}
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        ...
                        isPreviewing
                          ? "opacity-90 shadow-lg ring-2 ring-white/30"
                          : "hover:opacity-90 cursor-grab active:cursor-grabbing",
                    ...
                    {isPreviewing && (
                      <div className="absolute -top-6 left-0 bg-foreground text-background text-[10px] font-medium px-1.5 py-0.5 rounded whitespace-nowrap z-30 pointer-events-none shadow">
                        {dType === "resize"
                          ? dateKey(
                              addDays(
                                parseDate(t.due_date || t.start_date || ""),
                                dd,
                              ),
                            )
                          : dateKey(
                              addDays(
                                parseDate(t.start_date || t.due_date || ""),
                                dd,
                              ),
                            )}
                      </div>
            ...
            >
              ...
              {rows
                ...
                .flatMap((r) => {
                  const t = r.task;
                  return (t.blocked_by_ids || [])
                    .map((bid) => {
                      const blockerRow = rows.find(
                        (rr) => rr.type === "task" && rr.task.id === bid,
                      );
