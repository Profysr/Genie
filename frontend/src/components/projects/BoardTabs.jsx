    ...
    setConfirmState({
      ...
      onConfirm: () => {
        ...
        if (activeBoardId === board.id) {
          const fallback = boards.find(
            (b) => b.id !== board.id && b.is_default,
          );
          if (fallback) onSelectBoard(fallback);
        }
      },
  ...
  return (
    <div className="flex items-center flex-shrink-0 max-w-[40%]">
      ...
      >
        {boards.map((board) => {
          const meta =
            BOARD_TYPE_META[board.board_type] || BOARD_TYPE_META.kanban;
            ...
            >
              ...
              {menuOpen === board.id && (
                <>
                  ...
                  />
                  <div className="absolute top-full left-0 mt-1 w-44 z-50 bg-popover border rounded-md shadow-popover py-1 text-sm">
                    <button
