...
function resolveMember(member) {
  ...
  const name     = member.user?.full_name || member.full_name || member.user?.email || member.email || "Unknown";
  const avatar   = member.user?.avatar   || member.avatar;
  return { userId, name, avatar };
}

export default function SprintSwimLanes({ tasks, statuses, members, onTaskClick, labelsById }) {
  ...
  const cellTasks = (taskList, statusId) =>
    taskList.filter(t => t.status_id === statusId).sort((a, b) => a.order - b.order);

  const hasTasks = laneData.some(l => l.tasks.length > 0) || unassigned.length > 0;
  ...
  return (
    <div className="flex-1 overflow-auto">
      ...
      <div style={{ minWidth: `${192 + statuses.length * 212}px` }}>
          ...
          {statuses.map(status => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs font-semibold truncate">{status.name}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* ── Member rows ── */}
        {laneData.map(({ member, userId, name, avatar, tasks: memberTasks }) => {
          const doneCount = memberTasks.filter(t =>
            statuses.find(s => s.id === t.status_id && s.is_done)
          ).length;

          return (
            <div key={member.id || userId} className="flex border-b hover:bg-muted/20 transition-colors">
              {/* Person cell */}
              ...
              {statuses.map(status => {
                ...
                return (
                  ...
                  >
                    {cell.length > 0
                      ? cell.map(task => (
                          <MiniTaskCard key={task.id} task={task} onClick={onTaskClick} />
                        ))
                      : (
