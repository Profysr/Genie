    ...
    preview: (
      <div className="space-y-1 h-10 overflow-hidden">
        {[100, 100, 100].map((_, i) => (
          <div
            key={i}
            className="h-2.5 rounded bg-muted-foreground/15 flex items-center px-1.5 gap-1"
          >
    ...
    const payload = {
      ...
      name:
        ...
        (selectedTemplate?.name ??
          BOARD_TYPES.find((t) => t.id === selectedType)?.label ??
          "New Board"),
                  ...
                  {templates.map((tmpl) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (!boardName) setBoardName(tmpl.name);
                      }}
                      className={cn(
                        "text-left p-3 rounded-lg border transition-all",
