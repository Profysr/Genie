    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-separate border-spacing-1">
          <thead>
            <tr>
              <th className="text-left text-muted-foreground font-normal w-28 pb-1">
                Member
              </th>
              {dates.map((d) => (
                <th
                  key={d}
                  className="text-center text-muted-foreground font-normal pb-1 min-w-[36px]"
                >
                  <div>{formatDay(d)}</div>
                  <div className="text-[10px] opacity-70">{formatDate(d)}</div>
                </th>
              ))}
              <th className="text-center text-muted-foreground font-normal pb-1 pl-1">
                Total
              </th>
            </tr>
          </thead>
      ...
      <div className="flex items-center gap-1 mt-3">
        ...
        {[0, 0.2, 0.4, 0.7, 0.95].map((ratio, i) => (
          <div
            ...
            className={cn(
              "w-4 h-4 rounded",
              intensityClass(ratio * maxVal, maxVal),
            )}
          />
        ))}
