...
export default function Sidebar({
  ...
  return (
    ...
    >
      {/* Workspace header — doubles as the collapse toggle */}
      <button
        onClick={onToggleCollapse}
        title={collapsed ? `Expand — ${workspace?.name}` : "Collapse sidebar"}
        className={cn(
          "flex items-center gap-2.5 w-full border-b border-border/40 hover:bg-accent/50 transition-colors group",
      ...
      >
        ...
        {!collapsed && (
          <>
            ...
            <ChevronsLeft className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
          </>
        )}
      </button>
      ...
      <nav className="flex-1 overflow-y-auto py-1 px-1.5">
        {collapsed ? (
          // Collapsed: icon-only, root items only, no sub-items
          ...
          // Expanded: full nav with collapsible sub-items
          navGroups.map((group, gi) => (
            <div key={gi} className={gi > 0 ? "mt-3" : ""}>
              ...
              <div className="space-y-1">
                {group.items.map(
                  ({ to, icon: Icon, label, key, end, collapsible }) => {
