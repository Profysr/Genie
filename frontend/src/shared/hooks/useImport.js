...
export function useImportSources(workspaceId) {
  return useQuery({
    ...
    queryFn: () =>
      api
        .get(`/api/workspaces/${workspaceId}/import/sources/`)
        .then((r) => r.data),
    enabled: !!workspaceId,
    staleTime: Infinity,
  });
...
export function useImportJobs(workspaceId) {
  return useQuery({
    ...
    queryFn: () =>
      api
        .get(`/api/workspaces/${workspaceId}/import/jobs/`)
        .then((r) => r.data),
    enabled: !!workspaceId,
    staleTime: Infinity,
  });
...
function _useImportJob(workspaceId, jobId) {
  return useQuery({
    ...
    refetchInterval: (data) => (data?.status === "importing" ? 2000 : false),
  });
}
...
export function useUploadImport(workspaceId) {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["import", workspaceId, "jobs"] }),
  });
}
...
export function useUpdateImportMapping(workspaceId) {
  ...
  return useMutation({
    mutationFn: ({ jobId, field_mapping }) =>
      api
        .patch(`/api/workspaces/${workspaceId}/import/jobs/${jobId}/`, {
          field_mapping,
        })
        .then((r) => r.data),
    onSuccess: (_, { jobId }) =>
