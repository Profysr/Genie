...
export const useBoardFields = (workspaceId, boardId) =>
  useQuery({
    ...
    queryFn: () =>
      api
        .get(`/api/workspaces/${workspaceId}/boards/${boardId}/fields/`)
        .then((r) => r.data),
...
export const useCreateField = (workspaceId, boardId) => {
  ...
  return useMutation({
    mutationFn: (data) =>
      api
        .post(`/api/workspaces/${workspaceId}/boards/${boardId}/fields/`, data)
        .then((r) => r.data),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: fieldsKey(workspaceId, boardId) }),
  });
};
...
export const useDeleteField = (workspaceId, boardId) => {
  ...
  return useMutation({
    mutationFn: (fieldId) =>
      api.delete(
        `/api/workspaces/${workspaceId}/boards/${boardId}/fields/${fieldId}/`,
      ),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: fieldsKey(workspaceId, boardId) }),
  });
...
export const useUpsertFieldValue = (workspaceId, boardId, taskId) => {
  ...
  return useMutation({
    mutationFn: ({ field_id, value }) =>
      api
        .post(
          `/api/workspaces/${workspaceId}/boards/${boardId}/tasks/${taskId}/field-values/`,
          { field_id, value },
        )
        .then((r) => r.data),
    onSuccess: (updated) => {
      qc.setQueryData(["task-detail", workspaceId, boardId, taskId], (old) => {
        if (!old) return old;
        const existing = old.field_values?.find(
          (fv) => fv.field.id === updated.field.id,
        );
        return {
          ...old,
          field_values: existing
            ? old.field_values.map((fv) =>
                fv.field.id === updated.field.id ? updated : fv,
              )
