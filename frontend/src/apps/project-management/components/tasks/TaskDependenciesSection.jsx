...
import {
  ...
  useAddDependency,
  useRemoveDependency,
} from "@/apps/project-management/hooks/useDependencies";
    ...
    <div className="flex items-center gap-2 group px-2 py-1.5 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
      ...
      {task?.status_detail && (
        <span
          ...
          style={{
            backgroundColor: task.status_detail.color + "20",
            color: task.status_detail.color,
          }}
        >
  ...
  return (
    <div className={cn("rounded-lg border overflow-hidden", borderCls, bgCls)}>
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <span
          className={cn(
            "text-[11px] font-semibold uppercase tracking-wider flex-1",
            labelCls,
          )}
        >
          {label}
        ...
        <button
          ...
          className={cn(
            ...
            showPicker
              ? `${iconCls} bg-black/5 dark:bg-white/5`
              : `${labelCls} hover:bg-black/5 dark:hover:bg-white/5`,
          )}
        >
...
export default function TaskDependenciesSection({
  ...
  const { data: deps = { blocked_by: [], blocking: [] } } = useDependencies(
    workspaceId,
    boardId,
    taskId,
  );
  ...
  const blockedByIds = new Set(
    deps.blocked_by.map((d) => d.task?.id).filter(Boolean),
  );
  const blockingIds = new Set(
    deps.blocking.map((d) => d.task?.id).filter(Boolean),
  );
