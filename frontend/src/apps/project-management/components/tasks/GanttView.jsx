...
import GanttCanvas from "@/apps/project-management/components/tasks/GanttCanvas";
import {
  useGanttModel,
  computeRange,
  computeCriticalPath,
  ...
  GROUP_H,
  ROW_H,
} from "@/apps/project-management/hooks/useGanttModel";
...
export default function GanttView({
        ...
        if (dragRef.current) {
          ...
          if (Math.abs(deltaDays) > 0) {
            ...
            if (dragRef.current.type === "move") {
              ...
              if (base)
                updates.start_date = dateKey(
                  addDays(parseDate(base), deltaDays),
                );
            } else {
  ...
  const onDriverMouseMove = useCallback(
    (e) => {
      ...
      else if (hit.dragType === "resize" || hit.dragType === "resize-left")
        driver.style.cursor = "ew-resize";
