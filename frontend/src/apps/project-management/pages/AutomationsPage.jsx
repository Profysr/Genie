...
import {
  ...
  useUpdateAutomation,
  useDeleteAutomation,
} from "@/apps/project-management/hooks/useAutomations";
...
const AUTOMATION_TEMPLATES = [
  ...
  },
  {
    ...
    actions: [
      {
        type: "post_comment",
        payload: { body: "✅ This task has been marked as done." },
      },
    ],
  },
  {
    ...
    actions: [
      {
        ...
        payload: { message: "You have been assigned a task." },
      },
    ],
  },
];
...
export default function AutomationsPage() {
  ...
  const { data: rules = [], isLoading } = useAutomations(workspaceId, boardId);
  ...
  const addAction = () =>
    setDraft((d) => ({
      ...d,
      actions: [...d.actions, { type: "change_status", payload: {} }],
    }));
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-3xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Rules that run automatically when task events happen.
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {AUTOMATION_TEMPLATES.map((tpl, i) => (
                <button
                  ...
                  className="text-left p-4 rounded-md border hover:border-primary/30 hover:bg-primary/5 transition-colors"
                >
                  <p className="font-medium text-sm">{tpl.name}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    When:{" "}
                    {
                      TRIGGER_OPTIONS.find((t) => t.value === tpl.trigger.type)
                        ?.label
                    }
                  </p>
                </button>
        ...
        {/* Rule builder */}
        {building && (
          <div className="border rounded-md p-5 mb-6 bg-card space-y-5">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setDraft((d) => ({ ...d, trigger: { type: e.target.value } }))
                }
              >
            ...
            <div>
              ...
              {draft.conditions.length === 0 && (
                <p className="text-xs text-muted-foreground italic">
                  No conditions — rule fires for every matching trigger.
                </p>
              )}
              <div className="space-y-2">
                {draft.conditions.map((cond, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        updated[i] = {
                          ...
                          operator: CONDITION_OPERATORS[e.target.value][0],
                          value: "",
                        };
                    ...
                    >
                      {(CONDITION_OPERATORS[cond.field] || []).map((op) => (
                  ...
                  <div key={i} className="flex items-center gap-2">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        updated[i] = { type: e.target.value, payload: {} };
                        setDraft((d) => ({ ...d, actions: updated }));
                      }}
                    >
                      {ACTION_TYPES.map((a) => (
        ...
        {rules.length === 0 && !building ? (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Create a rule or start from a template above.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {rules.map((rule) => (
              <RuleCard
                ...
                onToggleExpand={() =>
                  setExpandedRule((e) => (e === rule.id ? null : rule.id))
                }
                onToggleActive={() =>
                  updateRule.mutate({
                    id: rule.id,
...
function TemplatePicker({ onSelect }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-9 z-50 w-72 bg-popover border rounded-md shadow-lg p-1.5">
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0" onClick={onToggleExpand}>
          ...
          <p className="text-xs text-muted-foreground">
            When: {triggerLabel} · Fired {rule.fire_count}×
          </p>
        </div>
        <button
          onClick={onDelete}
