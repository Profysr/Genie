...
export const useProject = (workspaceSlug, projectId) =>
  useQuery({
    ...
    queryFn: () => api.get(`/api/workspaces/${workspaceSlug}/projects/${projectId}/`).then((r) => r.data),
...
export const useCreateProject = (workspaceSlug) => {
  ...
  return useMutation({
    mutationFn: (data) => api.post(`/api/workspaces/${workspaceSlug}/projects/`, data).then((r) => r.data),
...
export const useUpdateProject = (workspaceSlug, projectId) => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (data) => api.patch(`/api/workspaces/${workspaceSlug}/projects/${projectId}/`, data).then((r) => r.data),
    onSuccess: () => {
...
export const useDeleteProject = (workspaceSlug) => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (projectId) => api.delete(`/api/workspaces/${workspaceSlug}/projects/${projectId}/`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["projects", workspaceSlug] }),
  });
};
