...
export function useGanttModel(tasks, sprints, collapsedSet, statuses = []) {
  return useMemo(() => {
    ...
    // Build lookup maps up-front so every downstream access is O(1).
    // statusMap: id → status object (replaces statuses.find() inside the loop)
    // statusOrder: id → position index (for sorting status groups)
    const statusMap = new Map(statuses.map((s) => [s.id, s]));
    const statusOrder = new Map(statuses.map((s, i) => [s.id, i]));

    // Single O(N) pass to bucket tasks — avoids running tasks.filter() once
    // per sprint (which would be O(N × M) for N tasks and M sprints).
    const bySprintId = new Map();
    const nonSprintDated = [];
    const undated = [];

    for (const task of tasks) {
      if (task.sprint_id) {
        if (!bySprintId.has(task.sprint_id)) bySprintId.set(task.sprint_id, []);
        bySprintId.get(task.sprint_id).push(task);
      } else if (task.start_date || task.due_date) {
        nonSprintDated.push(task);
      } else {
        undated.push(task);
      }
    }
    ...
    for (const sprint of sortedSprints) {
      // Only sprint tasks that have at least one date are shown on the canvas.
      const sprintTasks = (bySprintId.get(sprint.id) ?? [])
    ...
    if (nonSprintDated.length > 0) {
      ...
      for (const statusId of sortedStatusIds) {
        ...
        // O(1) map lookup instead of a linear statuses.find() on every iteration
        const status = statusMap.get(statusId) ?? null;
